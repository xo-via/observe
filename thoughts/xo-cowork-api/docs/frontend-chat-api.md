...
| `GET /api/chat/stream/{stream_id}` | Open the SSE stream for a `stream_id`. Yields `session-created` then `text-delta*` then `done`. |
...
| `POST /ask_question_streaming` | LEGACY SSE. Different event shape from `/api/chat/*`. |

> [!WARNING]
> **Use `/api/chat/*` for everything new.** The legacy `/ask_question*` pair is preserved for backward compatibility but uses a different event vocabulary, a different session model, and bypasses the project-tied sessions tree.
...
sequenceDiagram
  ...
  participant R as Runtime (OpenClaw / Claude Code / Codex)

  C->>A: POST /api/chat/prompt {text, session_id?, agent_name?, ...}
  Note over A: enqueue stream_info into<br/>active_streams[stream_id]
  A-->>C: {stream_id, session_id}
  C->>A: GET /api/chat/stream/{stream_id} (EventSource)
  Note over A: pop stream_info<br/>spawn producer task
  A->>R: start turn (subprocess or HTTP)
  R-->>A: streamed tokens
  A-->>C: event: session-created (new turns only)
  loop tokens flow
    A-->>C: event: text-delta
  end
  A-->>C: event: heartbeat (during silent gaps)
  R-->>A: completion
  A-->>C: event: done
  Note over A: server closes stream
```
...
```mermaid
flowchart TD
  in[POST /api/chat/prompt] --> trim{text empty<br/>after trim?}
  trim -- yes --> err400([400 Empty message])
  trim -- no --> pick[resolve agent_name]
  pick --> override{body.agent_name set?}
  override -- yes --> use_explicit[use body.agent_name]
  override -- no --> hassess{body.session_id set?}
  hassess -- yes --> detect[find_session_backend session_id<br/>scans xo-projects + openclaw paths]
  detect --> detok{detected?}
  detok -- yes --> use_detected[use detected backend]
  detok -- no --> use_env[fallback to AGENT_NAME env<br/>default 'openclaw']
  hassess -- no --> use_env
  use_explicit --> resolve
  use_detected --> resolve
  use_env --> resolve
  resolve[resolve agent_id from body.agent_id<br/>or workspace path heuristic]
  resolve --> branch{agent_name?}
  branch -- openclaw + new session --> oc_new[bootstrap session via gateway<br/>poll up to 20s for session_id]
  oc_new --> oc_check{session file found?}
  oc_check -- yes --> ok200[200 stream_id + session_id]
  oc_check -- no --> okNull[200 stream_id + session_id=null]
  branch -- openclaw + resume --> oc_resume[find_session_key]
  oc_resume --> oc_resume_check{found?}
  oc_resume_check -- yes --> ok200
  oc_resume_check -- no --> err404([404 Session not found])
  branch -- claude_code / other --> cc_new[generate session_id synchronously]
  cc_new --> ok200
```
...
### 3.0 Wire shape: OpenClaw direct path
...
sequenceDiagram
  ...
  participant C as Client
  participant A as cowork-api
  participant OC as OpenClaw gateway (~/.openclaw)
  participant FS as Project tee files

  C->>A: GET /api/chat/stream/{stream_id}
  Note over A: pop active_streams[stream_id]<br/>agent_name == "openclaw"
  alt new session
    Note over A: emit_prefetched_sse(stream_id)
    A->>OC: start turn (POST gateway)
    OC-->>A: bootstrap done (session_id ready)
    A-->>C: event: session-created {session_id}
    A->>OC: GET stream (SSE)
    OC-->>A: text chunks
    A-->>C: event: text-delta {text, session_id}
  else existing session
    Note over A: stream_openclaw_to_sse(stream_id)
    A->>OC: GET stream (SSE)
    OC-->>A: text chunks
    A-->>C: event: text-delta {text, session_id}
  end
  ...
  OC-->>A: turn complete
  Note over A,FS: project tee writes<br/>~/xo-projects/&lt;pid&gt;/.xo/sessions/<br/>(if agent_id is known)
  A-->>C: event: done {finish_reason, session_id}
```

### 3.0a Wire shape: dispatcher path (claude_code / other adapters)
...
sequenceDiagram
  ...
  participant CC as Claude Code CLI subprocess

  C->>A: GET /api/chat/stream/{stream_id}
  Note over A: pop active_streams[stream_id]<br/>agent_name == "claude_code"
  A->>D: dispatch(stream_id)
  D->>CC: spawn `claude` subprocess<br/>cwd = ~/xo-projects/<agent_id><br/>--session-id + /skill-name prefix
  A-->>C: event: session-created {session_id}
  loop tokens
    CC-->>D: JSONL line
    D-->>A: normalized text-delta
    A-->>C: event: text-delta {text}
  end
  loop idle &gt;20s
    A-->>C: event: heartbeat {}
  end
  ...
  alt success
    ...
    A-->>C: event: agent-error {error_message}<br/>(no done follows)
  end
```
...
```mermaid
flowchart TD
  in[GET /api/chat/stream/stream_id] --> q1{active_streams<br/>has stream_id?}
  q1 -- yes --> normal[normal path:<br/>pop entry, start producer,<br/>record in _recently_started]
  q1 -- no --> q2{_recently_started<br/>has stream_id?}
  q2 -- yes --> q3{age &lt;<br/>_RECENTLY_STARTED_TTL = 600s?}
  q3 -- yes --> wait[wait on done_event<br/>up to 300s]
  wait --> emit[emit session-created id=1<br/>then done id=2 with session_id only<br/>NO finish_reason field]
  q3 -- no --> notfound[emit error: Stream not found<br/>and close]
  q2 -- no --> notfound
  normal --> turn_done{turn finishes?}
  turn_done -- yes --> _ne[set done_event;<br/>any concurrent reconnect resolves]
```
...
sequenceDiagram
  participant C as Client
  participant A as cowork-api
  C->>A: POST /api/chat/abort {stream_id}
  Note over A: active_streams.pop(stream_id, None)<br/>(in-flight runtime subprocess NOT killed)
  A-->>C: 200 {ok: true}
```
...
sequenceDiagram
  ...
  participant C as Client
  participant A as cowork-api
  participant R as Runtime (Claude Code subprocess)
  participant S as xo-swarm-api
  C->>A: POST /ask_question {project_name, question, ...}
  Note over A: lookup session_store[project_name]<br/>(returns session_id or None → new session)
  A->>R: run turn, collect full response
  R-->>A: full assistant text
  Note over A: session_store[project_name] = session_id
  A->>S: POST /chat/add_message (user + assistant pair)<br/>using stored Bearer token
  S-->>A: ok
  A-->>C: {id: null, message, project_id, user_id, session_id, is_new_session, timestamp}
```
...
sequenceDiagram
  ...
  participant S as xo-swarm-api
  C->>A: POST /ask_question_streaming
  A->>R: start turn
  loop tokens
    R-->>A: token
    A-->>C: data: {type: "token", token: "..."}
  end
  alt success
    R-->>A: done
    Note over A: session_store[project_name] = session_id
    A->>S: POST /chat/add_message (user + assistant pair)
    A-->>C: data: {done: true}
  else error mid-stream
    A-->>C: data: {type: "error", error: "..."}<br/>or data: {error: "..."}
    Note over A: session NOT stored,<br/>swarm NOT notified
  end
```

...
```mermaid
flowchart TB
  subgraph Client["xo-cowork (Tauri desktop)"]
    UI[chat UI / files / connectors / settings]
  end

  subgraph API["xo-cowork-api — FastAPI :5002"]
    direction TB
    subgraph TopRouters["Top-level routers"]
      RAuth["/xo-auth/*"]
      RClaude["/claude/*"]
      RCodex["/codex/*"]
      ROCUsage["/openclaw/usage/*"]
    end
    subgraph CoworkAgent["routers/cowork_agent/*"]
      RChat["/api/chat/*"]
      RSess["/api/sessions/*<br/>/api/messages/*"]
      RAgents["/api/agents/*"]
      RConfig["/api/config/*"]
      RFiles["/api/files/*"]
      RSecrets["/api/secrets/*"]
      RUsage["/api/usage"]
      RConn["/api/connectors/{gdrive,onedrive,github,vercel,manus}/*"]
      RMisc["/api/{onboarding,channels,…} + misc/stubs"]
    end
    subgraph Services["services/cowork_agent/*"]
      Disp[AgentDispatcher]
      OCAd[OpenclawAdapter]
      CCAd[ClaudeCodeAdapter]
      ConnSvc[Connector services<br/>gdrive_rclone, onedrive_rclone,<br/>github_connector + cli_auth,<br/>vercel_connector, manus_connector,<br/>rclone_oauth_lock]
      AuthSt[(in-memory<br/>auth_state +<br/>active_streams)]
    end
  end

  subgraph Runtimes["Local runtimes"]
    OCGW[OpenClaw HTTP gateway :18789]
    CCli[claude CLI subprocess]
    CodexCli[codex CLI subprocess]
  end

  subgraph Externals["External / on-disk"]
    Rclone[rclone CLI + rclone.conf]
    GH[gh CLI + api.github.com]
    Vercel[api.vercel.com + OAuth]
    Manus[api.manus.ai]
    Disk[(~/.openclaw/agents/*/sessions/*.jsonl<br/>~/.claude/projects/*/sid.jsonl<br/>~/xo-projects/pid/ — canonical layout<br/>~/.xo-cowork/state.json)]
  end

  Swarm[(xo-swarm-api<br/>Clerk auth, add_message,<br/>get_messages, usage/report)]

  UI -- "HTTP/SSE :5002" --> TopRouters
  UI -- "HTTP/SSE :5002" --> CoworkAgent
  TopRouters --> AuthSt
  RChat --> Disp
  Disp --> OCAd
  Disp --> CCAd
  ...
  OCGW --> Disk
  CCli --> Disk
  TopRouters -- "Bearer (XO_API_KEY or consumed token)" --> Swarm
  CoworkAgent -- "Bearer" --> Swarm
```

The ASCII diagram below conveys the same shape with explicit layering; both are kept in the wiki because they reinforce different mental models.

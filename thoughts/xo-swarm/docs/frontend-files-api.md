...
│ POST /api/files/mkdir          │ Create a directory; with scaffold:true, build a    │
│                                │ canonical xo-project tree from project_template/. │
└────────────────────────────────┴───────────────────────────────────────────────────┘
```
...
- Symlinks are followed by `Path.resolve()`. A symlink under `$HOME` pointing outside is a known leak (treat the workspace as the trust boundary).
